use lit_rust_sdk::{LitNodeClient, LitNodeClientConfig, LitNetwork};
...
async fn test_connect_to_datil_dev() {
    ...
    match client.connect().await {
        Ok(()) => {
            ...
            println!("\nNetwork State:");
            println!("  Network Public Key: {}", state.network_pub_key.as_ref().unwrap());
            println!("  Subnet Public Key: {}", state.subnet_pub_key.as_ref().unwrap());
            println!("  Latest Blockhash: {}", state.latest_blockhash.as_ref().unwrap());
